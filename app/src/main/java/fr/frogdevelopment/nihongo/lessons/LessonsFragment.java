...
//    }
    ...
    private void initPacksPresent() {
        ...
        packs = new HashSet<>(Arrays.asList(packsSaved != null ? packsSaved.split(";") : new String[0]));
    }
...
//                                lessons.add(new Lesson(skuDetails, true, checkPackPresent(skuDetails.getSku())));
    ...
    private void insertData(File file, final Lesson lesson) {
        ...
        try {
            ...
            final Iterable<CSVRecord> records = CSVFormat.TDF.withHeader().withSkipHeaderRecord().parse(in);
            String[] insert;
        ...
        NihonGoContentProvider contentProvider = (NihonGoContentProvider) getActivity().getContentResolver().acquireContentProviderClient(".NihonGoContentProvider").getLocalContentProvider();
        contentProvider.insertDataCompileStatement(inserts);
